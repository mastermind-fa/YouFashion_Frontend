  ...
  wishlist.forEach((item) => {
    const productCard = `
      <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <img src="https://you-fashion-backend.vercel.app${item.product.image}" alt="${item.product.name}" class="w-full h-48 object-cover">
        <div class="p-6">
...
async function addToCart(productId) {
  try {
    const response = await fetch("https://you-fashion-backend.vercel.app/products/cart/", {
      method: "POST",
      headers: {
